...
import type { AdminTop100CategoryDto } from '~/types/api'

definePageMeta({ layout: 'admin', middleware: ['auth', 'admin'] })
useSeoMeta({ title: 'فئات تحدي الـ100 - جولة' })

const { listTop100AdminCategories, createTop100AdminCategory, updateTop100AdminCategory, deleteTop100AdminCategory } = useAdminApi()
const toast = useToast()

const categories = ref<AdminTop100CategoryDto[]>([])
...
async function load() {
  loading.value = true
  try {
    categories.value = await listTop100AdminCategories()
  } catch {
...
const editingCategory = ref<AdminTop100CategoryDto | null>(null)
const formName = ref('')
const formIcon = ref('')
...
function openEdit(category: AdminTop100CategoryDto) {
...
async function save() {
  ...
  try {
    ...
    if (editingCategory.value) {
      await updateTop100AdminCategory(editingCategory.value.id, formName.value.trim(), icon, description)
      toast.add({ title: 'تم تحديث الفئة', color: 'success' })
    } else {
      await createTop100AdminCategory(formName.value.trim(), icon, description)
...
async function remove(category: AdminTop100CategoryDto) {
  deletingId.value = category.id
  try {
    await deleteTop100AdminCategory(category.id)
    ...
    >
      ...
      >
        <div class="flex items-center justify-between gap-3">
          <NuxtLink
            :to="`/admin/top100/categories/${category.id}`"
