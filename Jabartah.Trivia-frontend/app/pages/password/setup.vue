...
import type { PasswordCategoryDto, TeamSetupInput } from '~/types/api'

const { listPasswordCategories, createPasswordGameSession } = useApi()
...
const steps = [
  { title: 'اختر الفئة', description: 'اختر فئة أو أكثر لكلمات اللعبة' },
  { title: 'كوّن فريقك', description: 'سمّ فريقيك واختر لون وأيقونة كل فريق' },
  { title: 'امسح QR وحقّن', description: 'المُلمّح يمسح الرمز بجواله ليرى الكلمة سراً ويعطي تلميحاً واحداً' }
]

onMounted(async () => {
  try {
    categories.value = await listPasswordCategories()
  } catch {
...
async function startGame() {
  ...
  loading.value = true
  try {
    const result = await createPasswordGameSession(teams.value, selectedCategoryIds.value, roundsPerTeam.value)
    await navigateTo(`/password/game/${result.passwordGameSessionId}`)
  } catch {
  ...
  <div>
    <section class="bg-green-900 text-white text-center py-10 px-4">
      ...
      <p class="text-white/80 mt-1">
        لمّح لفريقك بكلمة واحدة فقط ليخمنوا الكلمة السرية
      </p>
    </section>

    <HowToPlaySteps
      title="كيف تلعب كلمة السر؟"
      :steps="steps"
    />
