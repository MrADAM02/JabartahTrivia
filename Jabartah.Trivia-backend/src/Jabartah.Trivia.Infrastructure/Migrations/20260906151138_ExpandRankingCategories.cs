...
{
    /// <inheritdoc />
    public partial class ExpandRankingCategories : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Clears the seeded Ranking tables so RankingDatabaseSeeder's one-time-only guard
            // (`if (await db.RankingCategories.AnyAsync()) return;`) sees an empty table again
            // and reseeds from ranking.seed.json (now 13 categories, 65 lists) on next startup.
