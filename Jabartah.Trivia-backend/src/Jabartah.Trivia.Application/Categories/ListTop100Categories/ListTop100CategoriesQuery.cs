...
public record Top100CategoryDto(Guid Id, string Name, string? Icon, string? Description, int ItemCount);
...
public class ListTop100CategoriesHandler(IApplicationDbContext db) : IQueryHandler<ListTop100CategoriesQuery, List<Top100CategoryDto>>
{
    public async Task<List<Top100CategoryDto>> Handle(ListTop100CategoriesQuery query, CancellationToken ct) =>
        await db.Top100Categories
            .OrderBy(c => c.Name)
            .Select(c => new Top100CategoryDto(
                c.Id,
                c.Name,
                c.Icon,
                c.Description,
                db.Top100ListItems.Count(i => db.Top100Lists.Any(l => l.Id == i.Top100ListId && l.Top100CategoryId == c.Id))
            ))
            .ToListAsync(ct);
}
